...
正好，我们采用一套模型聚合管理工具来统一管理多个 AI 模型账号的并发和会话。跑了一段时间都挺稳定，直到最近使用 OpenClaw 掉哟个模型时出现了问题：管理页面上某个账号的"并发数"一直在涨，最后触达上限，新请求全被拒了。但奇怪的是，另一个客户端（Claude Code）走同样的通道却始终显示 1。
